...
var builtinMetricMaps = map[string]map[string]ColumnMapping{
	...
	"pg_stat_all_tables": {
		"relid":               {LABEL, "OID of a table", nil, nil},
		...
		"n_mod_since_analyze": {GAUGE, "Estimated number of rows modified since this table was last analyzed", nil, nil},
		"last_vacuum":         {COUNTER, "Last time at which this table was manually vacuumed (not counting VACUUM FULL)", nil, nil},
		"last_autovacuum":     {COUNTER, "Last time at which this table was vacuumed by the autovacuum daemon", nil, nil},
		"last_analyze":        {COUNTER, "Last time at which this table was manually analyzed", nil, nil},
		"last_autoanalyze":    {COUNTER, "Last time at which this table was analyzed by the autovacuum daemon", nil, nil},
	...
	"pg_stat_replication": {
		...
		"pg_current_wal_lsn":       {GAUGE, "pg_current_xlog_location", nil, semver.MustParseRange(">=10.0.0")},
	...
	"pg_stat_replication": {
		{
			...
			`
			SELECT *,
				(case pg_is_in_recovery() when 't' then null else pg_wal_lsn_diff(pg_current_wal_lsn(), pg_lsn('0/0'))::float end) AS pg_current_wal_lsn,
